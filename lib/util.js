...
module.exports = function () {
    ...
    var getBranch = r.getBranch = function (gitanaConfig, branchId, callback) {
        console.log("Connecting to API server at: " + gitanaConfig.baseURL);

        Gitana.connect(gitanaConfig, function (err) {
            if (err) {
                // console.log("Failed to connect: " + JSON.stringify(err));
                return callback("Failed to connect: " + JSON.stringify(err));
            }
            ...
            }).readBranch(branchId || "master")
                // .trap(function (err) {
                //     console.log("getBranch trap. Failed to retrieve branch: " + JSON.stringify(err));
                //     return callback(err);
    ...
    var findFiles = r.findFiles = function (dirPath, name) {
        ...
        try {
            ...
            console.log("No local types defined. Folder not found: " + dirPath + ". error: " + error);
            return null;
        }
    };
